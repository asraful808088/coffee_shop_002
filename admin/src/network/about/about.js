import axios from "axios";
import { config } from "./../../config";
export function getAboutInfo(result) {
  axios
    .get(`${config.SERVER_HOST}/about/heroImage`)
    .then((res) => {
      if (result) {
        result(res.data);
      }
    })
    .catch((err) => {
      if (result) {
        result(err);
      }
    });
}

export function postHeroContext({ fullFile, title, description, webUrl }, result) {
  const formData = new FormData();

  if (fullFile) {
    formData.append("file-1", fullFile);
    if (webUrl) {
      formData.append("removeImage", JSON.stringify([webUrl]));
    }
  } else {
    formData.append("file-1", webUrl);
  }
  formData.append("title", title);
  formData.append("description", description);
  axios
    .post(`${config.SERVER_HOST}/about/heroImage`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    })
    .then((res) => {
      if (result) {
        result(res);
      }
    })
    .catch((err) => {
      if (result) {
        result(err);
      }
    });
}
